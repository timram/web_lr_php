...
(function (win) {
    ...
    function isRegistrationPage() {
        return window.location.href.indexOf('register') > 0;
    }

    function isBlogPage() {
        return window.location.href.indexOf('blog') > 0;
    }

    const getClosestParent = (child, parentAttributes) => {
        const parent = child.parentElement;
        if (!parent) {
            return null;
        }

        const isTargetParent = Object.keys(parentAttributes).every(k => {
            if (k === 'class') {
                return parent.classList.contains(parentAttributes[k]);
            }
            const parentAttr = parent.getAttribute(k);
            return parentAttr === parentAttributes[k];
        });
        ...
        return getClosestParent(parent, parentAttributes);
    };

    const sendRequest = ({ method, endpoint, data = null, json = false }) => new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = () => {
            ...
            return resolve(json ? JSON.parse(xhr.responseText) : xhr.responseText);
        };
    });
    ...
    function throttle(delay, func) {
        let startTime = null;
        return async (...params) => {
            const isCallable = !startTime || ((new Date().getTime() - startTime) > delay);
            if (isCallable) {
                await func(...params);
    ...
    function setNodeTextColor({ node, text, color }) {
        node.innerText = text;
        node.style.color = color;
    }

    const checkIfExists = throttle(500, async input => {
        const name = input.getAttribute('name');
        const value = input.value;
        const statusNode = input.parentElement.querySelector('.check-existing');
        if (value.length > 0 && statusNode) {
            setNodeTextColor({ node: statusNode, text: 'Проверка ...', color: '#000' });            
            const resp = await sendRequest({
                method: 'POST',
                endpoint: 'check-if-user-exists',
                data: { [name]: value }
            });
            console.log(resp);
            console.log(typeof resp);
            const { [name]: isExists } = JSON.parse(resp);
            if (isExists) {
                setNodeTextColor({ node: statusNode, text: `Такое ${name} уже занят!`, color: '#f00' });
            } else {
                setNodeTextColor({ node: statusNode, text: `${name} свободен!`, color: '#078e2f' });
            }
        }
    });

    function checkDataDuplicationWhileReg() {
        const inputs = ['login', 'email'];
        inputs.forEach((i) => {
            const node = document.querySelector(`input[name="${i}"]`);
            if (!!node) {
                node.addEventListener('blur', async function() {
                    await checkIfExists(this);
                });
            }
        });
    }

    function initTogglers(togglersSelector, action) {
    ...
    function initEditPostButtons() {
        initTogglers(
            '.edit-blog',
            target => {
                const container = target.parentElement;
                container.classList.toggle('opened');
                target.innerHTML = container.classList.contains('opened')
    ...
    function initShowCommentsButtons() {
        initTogglers(
            '.show-comments',
            target => {
                const container = target.parentElement;
    ...
    function initLeaveCommentButtons() {
        const buttons = document.querySelectorAll('.comment-creator-container button');
        buttons.forEach(b => b.addEventListener('click', throttle(1000, async e => {
            const comment = e.target.parentElement.querySelector('textarea').value;
            const blogID = b.dataset.blogId;
            const commentsContainer = getClosestParent(b, {
                class: 'blog-post'
            }).querySelector('.comments');
            console.log(commentsContainer);
            if (!!blogID && !!comment && comment.length > 0) {
                const resp = await sendRequest({
                    method: 'POST',
                    endpoint: '/comment-blog',
                    data: {
                        blogID,
                        content: comment
                    }
    ...
    function setBlogPostSubjectAndText(blogPost, subject, text) {
        blogPost.querySelector('.post-subject').innerHTML = subject;
        blogPost.querySelector('.post-body').innerHTML = text;
    }

    function initUpdatePostButtons() {
        const forms = document.querySelectorAll('form.editor');
        forms.forEach(f => f.addEventListener('submit', async e => {
            e.preventDefault();
            const blogPost = getClosestParent(f, { class: 'blog-post' });
            const updValues = {
                subject: f.querySelector('input[name="subject"]').value,
                text: f.querySelector('textarea[name="text"]').value
            };
            const updPost = await sendRequest({
                method: 'put',
            ...
            setBlogPostSubjectAndText(
                blogPost,
                updPost.subject,
                updPost.text
            );

            blogPost.querySelector('.btn.edit-blog').click();
        }));
    }

    function initBlogComments() {
        initShowCommentsButtons();
        initLeaveCommentButtons();
        initEditPostButtons();
        initUpdatePostButtons();
    }
