...
(function (win) {
    ...
    const sendRequest = ({ method, endpoint, data = null }) => new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = () => {
            ...
            return resolve(xhr.responseText.replace(/Array|\(|\)/g, '').trim());
        };
    });
    ...
    function initUpdatePostButtons() {
        ...
        forms.forEach(f => f.addEventListener('submit', () => {
            ...
            setBlogPostSubjectAndText(
                blogPost,
                f.querySelector('input[name="subject"]').value,
                f.querySelector('textarea[name="text"]').value
            );
        }));
    }
